...
class MockPlan:
    ...
    def __str__(self):
        return "Mock Plan ("+self.time+","+self.distance+")"
...
def AstarSearch(game: Game, actions: ActionPool, goals:List[Goal]):
    ...
    while type(bestplan) == MockPlan:
        ...
        print("------------------------------------"+str(max_depth)+"--------------------------------")
        print(game.state)
        bestplan = AStarDFS(game, goals, 0, ub_time, max_depth, plan_distance)
        ub_time += base
    return bestplan
...
def AStarDFS(game: Game, goals, current_time, ub_time, depth: int, ub_distance):
    ...
    bestplan = MockPlan(ub_time, ub_distance)
    depth2=depth-1
    ...
    potential = game.potential_end()
    if potential >= ub_time:
        print("Limitní čas...")
        return MockPlan(potential)
