...
class Status:
    ...
    def check(self, goal):
        if goal.name in self.shadow:
            print(str(goal.count)+"Pylons"+str(self.shadow[goal.name]))
    ...
    def waited(self,passed_time):
        ...
        print("MINERALS:" + str(self.minerals) + "/" + str(passed_time))
...
class Game:
    ...
    def refresh(self, old_time):
        ...
        print ("Nooooo"+str(old_time-self.CT()))
    ...
    def plan_next_action(self, action):
        ...
        print("Scheduling1:" + str(sch_a.duration))
        print(self.CT())
        print(self.state)
        sch_a.add_time(surplus_time)
        print("Scheduling2:" + str(sch_a.duration))
        self.window.plan_action(sch_a)
        self.state.start_action(sch_a)

        return self.CT() #TODO: Možná to má vracet něco jinýho
...
def AstarSearch(game, actions, goals):
    ...
    max_depth = 2*int(math.floor(ub_time / min))
    print("------------------------------------"+str(max_depth)+"--------------------------------")
    print(game.state)
...
def AStarDFS(game, goals, current_time, ub_time, depth):
    ...
    game.time_shift(new_current_time)
    finished_actions = game.refresh(bookmark)
    #print(str(finished_actions) + str(new_current_time))
    if (finished_actions == []):
    ...
    plan = AStarDFS(game, goals2, new_current_time, besttime,depth2)
    print("Pokračujeme zpět")
    if besttime > plan.time:
        print("Changing best plan" + str(depth) + str(bestplan) + "/" + str(plan))
        besttime = plan.time
        bestplan = plan
